...
def encoder_helper(dataframe: pd.DataFrame,
    ...
    try:
        ...
        for col_name in category_lst:
            ...
            run_logging.info("INFO : %s columns have been encoded", col_name)
...
def perform_feature_engineering(dataframe: pd.DataFrame,
    ...
    ----------
              ...
              test_ratio (Number) : Number of test portion from full dataset.
...
def classification_report_image(y_actual_dict: Dict,
    ...
    Args:
    ----------
            y_actual_dict (Dict) : Collect list of actual y train and test
...
def feature_importance_plot(model: Predictor,
    ...
    plt.savefig(
        f"{constant.path['IMG_RESULT_PATH']}/TreeBase Impurity Score",
        bbox_inches='tight')
    plt.close()
...
def train_models(x_train: np.ndarray,
    ...
    '''
    Selected model for training are LogistricRegression and RandomForest.
    and list of actions in train_modesl are as below
    - DO train and store model results: images + scores, and store models.
    - Hyperparameter have been tuned for RandomForest by GridSearch Method.
    - ROC Curve for both train and test data is saved in .png format.
    - Feature importance for random forest model have been calcualted in
    2 methods, SHAP value and built-in method in RandomForeast from Scikit-Learn.
